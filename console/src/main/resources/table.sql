...
CREATE TABLE IF NOT EXISTS `producer` (
  ...
  `permission` VARCHAR(64) NOT NULL DEFAULT 1 COMMENT '权限 1发送消息 2发送延迟消息',
...
CREATE TABLE IF NOT EXISTS `consumer` (
  ...
  `status` INT NOT NULL DEFAULT 1 COMMENT '状态值',
  `permission` VARCHAR(64) NOT NULL DEFAULT 1 COMMENT '权限 1消费消息 2消费延迟消息',
  `create_time` DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
  `update_time` DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '修改时间',
  PRIMARY KEY (`id`),
...
CREATE TABLE IF NOT EXISTS `destination` (
  ...
  `status` INT NOT NULL DEFAULT 1 COMMENT '状态值',
  `permission` VARCHAR(64) NOT NULL DEFAULT 1 COMMENT '权限 1接收消息 2接收延迟消息',
  `create_time` DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
  `update_time` DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '修改时间',
  PRIMARY KEY (`id`),
